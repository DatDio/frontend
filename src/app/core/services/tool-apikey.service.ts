...
import { ToolApiKey, ToolApiKeyCreate, ToolApiKeyGenerated, ToolApiKeyUpdate } from '../models/tool-apikey.model';
...
export class ToolApiKeyService {
    ...
    /**
     * Update existing Tool API Key
     */
    update(id: number | string, request: ToolApiKeyUpdate): Observable<ApiResponse<ToolApiKey>> {
        this.loaderService.show();
        return this.httpClient
            .put<ApiResponse<ToolApiKey>>(AdminToolApiKeyApi.UPDATE(id), request)
            .pipe(finalize(() => this.loaderService.hide()));
    }
